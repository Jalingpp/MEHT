...
for BC in ${mehtBC[*]}; do 
    go run $insertion_and_query_file $siMode $U4Num 32 $batch_size $mbtBN $BC $mehtBS $U4
    rm -rf data/levelDB
done

for BC in ${mehtBC[*]}; do 
    for file in ${S2[*]}; do
        go run $pure_insertion_file $siMode $U1Num 32 $batch_size $mbtBN $BC $mehtBS $U1
        go run $insertion_and_query_file $siMode $S2Num 32 $batch_size $mbtBN $BC $mehtBS $file
        rm -rf data/levelDB
    done
done
...
#             go run $pure_query_file $siMode $U3Num $thread $mbtBN $mehtBC $mehtBS $U3
#             rm -rf data/levelDB
...
#             rm -rf data/levelDB
#         fi
#         go run $insertion_and_query_file $siMode $RealNum $thread $batch_size $mbtBN $mehtBC $mehtBS $Real
#         rm -rf data/levelDB
#     done
#     #-------------------------------------------------------------------------------------------------------#
#     #test2 & test4
...
#         du -sh data/levelDB/PrimaryDB${U8Num[${i}]}$siMode >> dbSize.txt
#         du -sh data/levelDB/SecondaryDB${U8Num[${i}]}$siMode >> dbSize.txt
#         rm -rf data/levelDB
#     done
#     #-------------------------------------------------------------------------------------------------------#
#     #test3
...
#             go run $pure_insertion_file $siMode $U1Num 32 $batch_size $mbtBN $mehtBC $mehtBS $U1
#             go run $pure_query_file $siMode $S1Num 32 $mbtBN $mehtBC $mehtBS ${S1[${i}]} $screw_option
#             rm -rf data/levelDB
#         done
#     done
#     for ((i=0;i<${#S2[@]};++i)); do
#         for screw_option in ${screw[*]}; do
#             go run $pure_insertion_file $siMode $U1Num 32 $batch_size $mbtBN $mehtBC $mehtBS $U1
#             go run $insertion_and_query_file $siMode $S2Num 32 $mbtBN $mehtBC $mehtBS ${S2[${i}]} $screw_option
#             rm -rf data/levelDB
#         done
#     done
# done